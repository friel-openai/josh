...
fn bench_parse_exclude(c: &mut Criterion) {
    ...
    for (name, paths) in [("1000", &f.paths_1000), ("5000", &f.paths_5000)] {
        let spec = make_exclude_spec(paths);
        group.bench_with_input(BenchmarkId::new("parse_exclude", name), &spec, |b, spec| {
...
fn bench_optimize_exclude_and_pin(c: &mut Criterion) {
    ...
    let mut seed: u32 = 0;
    let mk_paths = |n: usize, seed: u32| -> Vec<PathBuf> {
        (0..n)
            .map(|i| {
                let dir = (i % 256) as u32;
            ...
            |b| {
                if fallback {
                    b.iter_custom(|_iters| {
                        ...
                        let repo = tx.repo();
                        let mut odb = repo.odb().expect("odb");
